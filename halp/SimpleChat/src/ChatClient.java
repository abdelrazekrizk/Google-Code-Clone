...
public class ChatClient extends AbstractClient
{
  ...
  public void handleMessageFromServer(Object msg) 
  {
	...
	{
		//for now accept all requests, in the future add some type of confirmation
   ...
   */
  public void handleMessageFromClientUI(String message) //good place to trapclient commands
  {
    try
    {
    	if(message.toString().charAt(0) == '#')
    	{
    		...
    		else if(message.indexOf("#changeuser") >= 0)
    		{
    			...
    			userName = newUser;
    			clientUI.display("Your selected username is now "+userName);
    		}
  ...
  private void sendFileToUser(String message)
  {
	...
	FileTransferRequest sf = new FileTransferRequest(filename, filesize, host, getPort()+1, userName, targetUser);
	try
	{
		sendToServer(sf);
	}
	...
	//currently the sending client will immediatly wait for a connection, in the future add a timeout or wait for confirmation
	FileTransfer ft = new FileTransfer(dir, filename, filesize, getInetAddress().toString(), getPort()+1, userName, targetUser);
	ft.start();
  }
