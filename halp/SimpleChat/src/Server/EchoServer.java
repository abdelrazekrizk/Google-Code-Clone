...
public class EchoServer extends AbstractServer 
{
  ...
  public void handleMessageFromClient(Object msg, ConnectionToClient client)
  {
  	...
  	else if(msg instanceof FileTransferRequest)
  	{
    	...
    	this.sendToAllClients(client.getInfo("userId") + ": " + msg);
  		...
  		{
  			serverUI.display("Message received: " + msg + " from " + client + "in group " + client.getInfo("group"));
    		this.sendToAllClientsInMyGroup(client.getInfo("group") +">> " + client.getInfo("userId") + ": " + msg, client.getInfo("group").toString());  		}
  ...
  if( message.toString().charAt(0) == '#')
  {
  	if(message.toString().indexOf("#quit") >= 0)
  	{
  		...
  		{
  			serverUI.display("Could not send message to server:" + e.toString());
  		}
  		try
  		{
  			close();
  	...
  	else if(message.toString().indexOf("#setyell") >= 0)
  	{
  		...
  		messageBody = messageBody.substring(start2+1, messageBody.length());
  		...
  		Thread[] clientThreadList = getClientConnections();
  		  	for (int i=0; i<clientThreadList.length; i++)
		    {
		    	if(	((ConnectionToClient)clientThreadList[i]).getInfo("userId") != null &&
		   	  		((ConnectionToClient)clientThreadList[i]).getInfo("userId").equals(targetUser))
		      	try
		      	{
		      		((ConnectionToClient)clientThreadList[i]).sendToClient("Your ability to yell is now "+messageBody);
		        	((ConnectionToClient)clientThreadList[i]).setInfo("canYell", messageBody);
		      	}
		        catch (Exception ex) {}	
		    }
  ...
  else
  {
    ...
    {
      serverUI.display
        ("Could not send message to server:" + e.toString());
  ...
  /****************Perform Command***************************/
   ...
   private void clientCommandLogin(Object msg, ConnectionToClient client)
   {
  		...
  		if(client.getInfo("userId")==null)
  		{
  			...
  			this.sendToAllClients(client.getInfo("userId")+" has just logged in");
  		}
  		else
  		{
  			try
   ...
   private void clientCommandQuit(Object msg, ConnectionToClient client)
   {
   		if(client.getInfo("userId")!=null)
   		{
   			this.sendToAllClients(client.getInfo("userId")+" has disconnected.");
   		}
  		else
  		{
   ...
   private void clientCommandJoin(Object msg, ConnectionToClient client)
   {
		...
		this.sendToAllClients(client.getInfo("userId")+" has just joined group "+group);
   ...
   private void clientCommandWhisper(Object msg, ConnectionToClient client)
   {
   		...
   		//find the space after the first word, which will be the target username
   		...
   		this.sendToAClient(client.getInfo("userId")+" whispers to you: "+tempMsg, targetUser);
   ...
   private void clientCommandYell(Object msg, ConnectionToClient client)
   {
   	...
   	if(client.getInfo("canYell")!=null && client.getInfo("canYell").equals("on"))
   	{
	   			...
	   			if(	((ConnectionToClient)clientThreadList[i]).getInfo("userId") != null &&
	   	  		...
	   	  		try
	   	  		{
	   	  			if(((ConnectionToClient)clientThreadList[i]).getInfo("group") !=null && 
	   	  				((ConnectionToClient)clientThreadList[i]).getInfo("group").equals(client.getInfo("group")))
	   	  				{
	   	  					((ConnectionToClient)clientThreadList[i]).sendToClient(client.getInfo("userId")+" is looking for you, run and hide.");
	   	  					try
	   	  					{
   ...
   public void sendToAllClientsInMyGroup(Object msg, String group)
  {
    ...
    for (int i=0; i<clientThreadList.length; i++)
    {
   	  ...
   	  if(	((ConnectionToClient)clientThreadList[i]).getInfo("group") != null &&
   	  		((ConnectionToClient)clientThreadList[i]).getInfo("group").equals(group))
      	try
      	{
  ...
  public void sendFileToAClient(FileTransferRequest req)
  {
	  ...
	  for (int i=0; i<clientThreadList.length; i++)
	  {
		...
		try
	   	{
	       	((ConnectionToClient)clientThreadList[i]).sendToClient(req);
	   	}
	    catch (Exception ex) {}	
	  }
  }
