...
public class ClientConsole extends JFrame implements ChatIF
{
	...
	public ClientConsole(String host, int port, String userId)
	{
		...
		public void actionPerformed(ActionEvent e)
		{
			if(!dir.getText().equals("") && !filename.getText().equals(""))
			{
				...
				{
					String message = "#sendfile" +" "+ targetuser.getText()+" " + dir.getText() +"|"+ filename.getText();;
					client.handleMessageFromClientUI(message);
				}
				else
				{
					client.clientUI.display("You must select a user to send the file.");
				}
			}
			else
			{
	...
	public void display(String message)
	{
		...
		messageList.scrollRectToVisible(new Rectangle(0, messageList.getHeight()-2,1,1));
