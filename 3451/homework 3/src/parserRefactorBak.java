...
public class parserRefactorBak extends PApplet {
	...
	public void mousePressed(){
		...
		if(!traceResults.isEmpty()){
			...
			for(TraceResult r : traceResults){
				...
				println(String.format("sceneObject[%d] (dist: %.2f) %s at %s [Normal: %s]", 
						sceneShapes.indexOf(r.intersectedShape), 
						r.intersection.length(), 
						r.intersectedShape, 
						r.intersection, 
						normal));
			}
			TraceResult r = Trace(source, direction);
			println(String.format("Result of the Trace: sceneObject[%d] (dist: %.2f) %s at %s", 
					sceneShapes.indexOf(r.intersectedShape), 
					r.intersection.length(), 
					r.intersectedShape, 
					r.intersection));
		...
		for(RayTracable shape : sceneShapes){
			...
			if(intersections != null){
				for(Vector v : intersections){
					if((nearestLoc == null || nearestLoc.vMinus(source).length() > v.vMinus(source).length())){
						nearestLoc = v;
						nearestShape = shape;
					}
				}
...
//			}
		...
		public Vector NormalAt(Vector v) {
			...
			if(abs(v.y - center.y) < EPSILON){
				...
				normal = new Vector(v.x - center.x, 0, v.z - center.z).normalize();
			}
