...
You can also run some example scripts/data files with ant. To see all the supplied ant targets type
	ant -p
at the command line in the mawk folder (that which contains build.xml). All targets listed with the word example are example targets (save copyExamples). To see what exactly is being passed, look in the build.xml file.
